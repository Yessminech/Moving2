...
Color::Color ColorSensor::get_color() {
    ...
    // read data
    auto data = buildHat.serial_write_read("port " + std::to_string(this->port) + " ; selonce 5");
    ...
    int hue, sat, val, norm;
    iss >> hue >> sat >> val >> norm;

    if (hue == 0 && sat == 0 && val == 0 && norm == 0) {
    ...
    norm = norm / 2;
    // decode data
    sat = static_cast<int>((static_cast<double> (sat) / static_cast<double> (norm)) * 255);
    val = static_cast<int>((static_cast<double> (val) / static_cast<double> (norm)) * 255);
    ...
    hue = static_cast<int>((static_cast<double> (hue) / static_cast<double> (norm)) * 255);
    */
