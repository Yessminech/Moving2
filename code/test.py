...
def get_best_action(Q_table, current_state, previous_state):
    ...
    best_action = np.argmax(Q_table[curr_col , curr_dist, : , prev_col , prev_dist]) #ToDo not sure
    logging.info(f"Best action for state {sample_state} and previous state {sample_prev_state} gis: {best_action}")
    #return best_action
